...
namespace euler013csharp
{
    class Program
    {
        #region input
        // again, the alt+select copy/paste to the rescue
        static readonly int[] map = new int[] 
        {
        ...
        static void Main(string[] args)
        {
            ...
            Console.WriteLine("The first ten digits of the sum of the following one-hundred 50-digit numbers");
            sw.Start();
            var sum1 = test();
            sw.Stop();
        ...
        /// <summary>
        /// i basically just translate how i do number addition to code, lawl, real life algorithm ftw
        /// add each "column" of digits(sum) plus carry over, mod 10, the remainder goes to this list
        /// the carry over is sum of abuve (sum + carry over(if any)) div 10
        /// repeat until last column(column order goes from right to left)
        /// so the total sum is the list(read from list[len-1] to list[0])
        /// </summary>
        /// <returns>first 10 digit of the sum</returns>
        static string test()
        {
